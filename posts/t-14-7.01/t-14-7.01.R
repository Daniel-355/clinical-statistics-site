...
advs_eot <- advs2 %>%
  ...
  summarise(n = n(),
            ...
            min = min(AVAL),
            max = max(AVAL)) %>%
  mutate(PRTFL = "End of Trt.")

advs3 <- rbind(advs_bl, advs_w24, advs_eot) %>%
  arrange(PARAM, ATPT, TRTP, PRTFL) %>%
  relocate(PRTFL, .before = n)
...
write_rtf(doc, file=glue('{output_lib}/t-14-7.01/files/14-7.01.rtf'))
