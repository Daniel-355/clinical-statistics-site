...
project_root <- if (file.exists("metadata/pk_spec.xlsx")) normalizePath(".") else if (file.exists("../metadata/pk_spec.xlsx")) normalizePath("..") else if (file.exists("../../metadata/pk_spec.xlsx")) normalizePath("../..") else stop("Cannot find metadata. Run from the project root, adam folder, or dataset folder.")

## ----r echo=TRUE, message=FALSE-----------------------------------------------
# ---- Load Specs for Metacore ----
metacore <- spec_to_metacore(file.path(project_root, "metadata", "pk_spec.xlsx"), quiet = TRUE) %>%
  select_dataset("ADER")
...
## ----r------------------------------------------------------------------------
dir <- file.path(project_root, "output", "adam")
...
source(file.path(project_root, "adam", "write_adam_excel.R"))
write_adam_excel(
  haven::read_xpt(file.path(dir, "ader.xpt")),
  "ADER",
  file.path(project_root, "adam", "ader", "files", "ader.xlsx")
)
...
write_adam_excel(
  haven::read_xpt(file.path(dir, "adee.xpt")),
  "ADEE",
  file.path(project_root, "adam", "ader", "files", "adee.xlsx")
)
...
write_adam_excel(
  haven::read_xpt(file.path(dir, "ades.xpt")),
  "ADES",
  file.path(project_root, "adam", "ader", "files", "ades.xlsx")
)
...
write_adam_excel(
  haven::read_xpt(file.path(dir, "adtrr.xpt")),
  "ADTRR",
  file.path(project_root, "adam", "ader", "files", "adtrr.xlsx")
)
