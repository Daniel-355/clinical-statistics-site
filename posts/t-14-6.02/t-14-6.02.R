...
adlbc2 <- adlbc %>%
  ...
  group_by(PARAM, TRTP, LBNRIND, .drop = FALSE) %>%
  summarise(N = n()) %>%
  group_by(PARAM, TRTP) %>%
  mutate(tot = sum(N)) %>%
  arrange(PARAM, TRTP)
...
for(i in seq(nrow(adlbh2)/9)) {
  ...
  if(all(mat[,1] == 0) & all(mat[,3] == 0)) {
    ...
    adlbh_pvals[i] <- num_fmt(round(fisher.test(mat, simulate.p.value = TRUE, B = 100000)$p.value, 3), size = 4, digits = 3, int_len = 1)
  }
}
...
# Write out the RTF
write_rtf(doc, file=glue('{output_lib}/t-14-6.02/files/14-6.02.rtf'))
