...
project_root <- if (file.exists("metadata/safety_specs.xlsx")) normalizePath(".") else if (file.exists("../metadata/safety_specs.xlsx")) normalizePath("..") else if (file.exists("../../metadata/safety_specs.xlsx")) normalizePath("../..") else stop("Cannot find metadata. Run from the project root, adam folder, or dataset folder.")
...
metacore <- spec_to_metacore(
  path = file.path(project_root, "metadata", "safety_specs.xlsx"),
...
write_adam_excel(
  haven::read_xpt(file.path(dir, "adsl.xpt")),
  "ADSL",
  file.path(project_root, "adam", "adsl", "files", "adsl.xlsx")
)
