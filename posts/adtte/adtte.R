...
project_root <- if (file.exists("metadata/onco_spec.xlsx")) normalizePath(".") else if (file.exists("../metadata/onco_spec.xlsx")) normalizePath("..") else if (file.exists("../../metadata/onco_spec.xlsx")) normalizePath("../..") else stop("Cannot find metadata. Run from the project root, adam folder, or dataset folder.")

## ----r read-specs-------------------------------------------------------------
# Load metacore specifications
metacore <- spec_to_metacore(file.path(project_root, "metadata", "onco_spec.xlsx")) %>%
...
write_adam_excel(
  haven::read_xpt(file.path(dir, "adtte.xpt")),
  "ADTTE",
  file.path(project_root, "adam", "adtte", "files", "adtte.xlsx")
)
