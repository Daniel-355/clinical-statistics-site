...
# No concomitant-medication ADaM dataset is present in the supplied package.
# Keep the table runnable and make the absence explicit instead of reading SDTM.
cm <- tibble(USUBJID = character(), CMCLAS = character(), CMDECOD = character())
...
write_rtf(doc, file=glue('{output_lib}/t-14-7.04/files/14-7.04.rtf'))
