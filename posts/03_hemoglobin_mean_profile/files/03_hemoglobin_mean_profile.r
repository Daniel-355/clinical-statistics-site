# =============================================================================
# PILOT clinical figure: Mean Hemoglobin over Time with 95% Confidence Intervals
#
...
stem <- "03_hemoglobin_mean_profile"
set.seed(2026082903)
...
original_csv <- paste(c(
  "USUBJID,TRTP,AVISIT,PARAMCD,TRTPN,AVISITN,AVAL",
  "1001,Placebo,Baseline,HGB,1,1,13.5",
  "1001,Placebo,Month,HGB,1,2,13.8",
  "1001,Placebo,Month,HGB,1,3,13.6",
  "1001,Placebo,Month,HGB,1,4,13.7",
  "1001,Placebo,Month,HGB,1,5,13.5",
  "1002,PILOT,Baseline,HGB,2,1,13.7",
  "1002,PILOT,Month,HGB,2,2,14.2",
  "1002,PILOT,Month,HGB,2,3,14.5",
  "1002,PILOT,Month,HGB,2,4,14.7",
  "1002,PILOT,Month,HGB,2,5,14.9",
  "1003,Placebo,Baseline,HGB,1,1,13.4",
  "1003,Placebo,Month,HGB,1,2,13.6",
  "1003,Placebo,Month,HGB,1,3,13.5",
  "1003,Placebo,Month,HGB,1,4,13.5",
  "1003,Placebo,Month,HGB,1,5,13.4",
  "1004,PILOT,Baseline,HGB,2,1,13.6",
  "1004,PILOT,Month,HGB,2,2,14",
  "1004,PILOT,Month,HGB,2,3,14.3",
  "1004,PILOT,Month,HGB,2,4,14.6",
  "1004,PILOT,Month,HGB,2,5,14.7",
  "1005,Placebo,Baseline,HGB,1,1,13.2",
  "1005,Placebo,Month,HGB,1,2,13.3",
  "1005,Placebo,Month,HGB,1,3,13.2",
  "1005,Placebo,Month,HGB,1,4,13.4",
  "1005,Placebo,Month,HGB,1,5,13.2",
  "1006,PILOT,Baseline,HGB,2,1,13.5",
  "1006,PILOT,Month,HGB,2,2,13.9",
  "1006,PILOT,Month,HGB,2,3,14.1",
  "1006,PILOT,Month,HGB,2,4,14.3",
  "1006,PILOT,Month,HGB,2,5,14.4",
  "1007,PILOT,Baseline,HGB,2,1,13.8",
  "1007,PILOT,Month,HGB,2,2,14.1",
  "1007,PILOT,Month,HGB,2,3,14.2",
  "1007,PILOT,Month,HGB,2,4,14.4",
  "1007,PILOT,Month,HGB,2,5,14.6"
), collapse = "\n")
...
# -----------------------------------------------------------------------------
stats_mean <- aggregate(AVAL ~ TRTP + AVISITN, simulated_data, mean)
stats_sd <- aggregate(AVAL ~ TRTP + AVISITN, simulated_data, sd)
stats_n <- aggregate(AVAL ~ TRTP + AVISITN, simulated_data, length)
names(stats_mean)[3] <- "MEANAVAL"; names(stats_sd)[3] <- "SDAVAL"; names(stats_n)[3] <- "N"
plot_data <- Reduce(function(x, y) merge(x, y, by = c("TRTP", "AVISITN")), list(stats_mean, stats_sd, stats_n))
plot_data$LCLM <- plot_data$MEANAVAL - qt(0.975, pmax(1, plot_data$N - 1)) * plot_data$SDAVAL / sqrt(plot_data$N)
plot_data$UCLM <- plot_data$MEANAVAL + qt(0.975, pmax(1, plot_data$N - 1)) * plot_data$SDAVAL / sqrt(plot_data$N)
plot_data$XPOS <- plot_data$AVISITN + ifelse(plot_data$TRTP == "Placebo", -0.05, 0.05)

finalplot <- ggplot2::ggplot(plot_data, ggplot2::aes(x = XPOS, y = MEANAVAL, color = TRTP, linetype = TRTP, shape = TRTP)) +
  ggplot2::geom_line(linewidth = 0.8) +
  ggplot2::geom_errorbar(ggplot2::aes(ymin = LCLM, ymax = UCLM), width = 0.1) +
  ggplot2::geom_point(size = 2.8) +
  ggplot2::scale_x_continuous("Analysis Visit", breaks = 1:5, labels = c("Baseline", "Month 1", "Month 2", "Month 3", "Month 4")) +
  ggplot2::scale_y_continuous("Hemoglobin (mg/dL)") +
  ggplot2::scale_color_manual(values = c("Placebo" = "#D55E00", "PILOT" = "#009E73"), name = "Treatment") +
  ggplot2::scale_linetype_manual(values = c("Placebo" = "dotted", "PILOT" = "solid"), name = "Treatment") +
  ggplot2::scale_shape_manual(values = c("Placebo" = 17, "PILOT" = 16), name = "Treatment") +
  ggplot2::labs(title = "Hemoglobin over Time (Mean ± 95% CI)", subtitle = "Safety Analysis Set") +
...
write_figure_rtf(png_path, rtf_path, "Mean Hemoglobin over Time with 95% Confidence Intervals", "14.1.3")
