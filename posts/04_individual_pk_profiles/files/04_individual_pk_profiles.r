# =============================================================================
# PILOT clinical figure: Individual Plasma Concentration-Time Profiles
#
...
stem <- "04_individual_pk_profiles"
set.seed(2026082904)
...
original_csv <- paste(c(
  "USUBJID,TRTP,PARAMCD,PARAM,TRTPN,TIME,AVAL",
...
simulated_data <- do.call(rbind, lapply(1:2, function(copy_number) {
  ...
  block$AVAL <- pmax(0, block$AVAL * exp(rnorm(nrow(block), 0, 0.08)))
  block
}))
...
# -----------------------------------------------------------------------------
finalplot <- ggplot2::ggplot(simulated_data, ggplot2::aes(x = TIME, y = AVAL, color = USUBJID, group = USUBJID)) +
  ggplot2::geom_line(show.legend = FALSE, linewidth = 0.7) +
  ggplot2::geom_point(show.legend = FALSE, size = 1.5) +
  ...
  ggplot2::labs(title = "Individual Plasma Concentration-Time Profiles", subtitle = "Pharmacokinetic Population") +
...
write_figure_rtf(png_path, rtf_path, "Individual Plasma Concentration-Time Profiles", "14.1.4")
