...
total_ABLFL1 <- comb%>%
  ...
  group_by(PARAM, VISIT, TRTP, BNRIND, .drop = FALSE) %>%
  summarise(N = n())
...
comb2 <- comb %>%
  filter(!is.na(VISIT), !is.na(TRTP), !is.na(BNRIND), !is.na(ANRIND), !is.na(PARAM)) %>%
  group_by(PARAM, VISIT, TRTP, BNRIND, ANRIND, .drop = FALSE) %>%
  summarise(N = n()) %>%
...
# Write out the RTF
write_rtf(doc, file=glue('{output_lib}/t-14-6.04/files/14-6.04.rtf'))
