# =============================================================================
# PILOT clinical figure: Subject-Level Percent Change in FEV1
#
...
original_csv <- paste(c(
  "USUBJID,AVISITN,PCHG,TRTP,PARAMCD",
  "101,0,0,Placebo,FEV1",
  "101,4,-10,Placebo,FEV1",
  "101,8,-20,Placebo,FEV1",
  "101,12,-30,Placebo,FEV1",
  "102,0,0,Drug PILOT,FEV1",
  "102,4,-25,Drug PILOT,FEV1",
  "102,8,-40,Drug PILOT,FEV1",
  "102,12,-50,Drug PILOT,FEV1",
  "103,0,0,Drug PILOT,FEV1",
  "103,4,-5,Drug PILOT,FEV1",
  "103,8,-15,Drug PILOT,FEV1",
  "103,12,-10,Drug PILOT,FEV1",
  "104,0,0,Placebo,FEV1",
  "104,4,-8,Placebo,FEV1",
  "104,8,-12,Placebo,FEV1",
  "104,12,-14,Placebo,FEV1",
  "105,0,0,Placebo,FEV1",
  "105,4,8,Placebo,FEV1",
  "105,8,20,Placebo,FEV1",
  "105,12,50,Placebo,FEV1"
), collapse = "\n")
...
simulated_data <- do.call(rbind, lapply(1:2, function(copy_number) {
  ...
  block$PCHG <- pmax(-80, pmin(80, block$PCHG + rnorm(nrow(block), 0, 3)))
  block
}))
...
# -----------------------------------------------------------------------------
label_index <- ave(simulated_data$AVISITN, simulated_data$USUBJID, FUN = function(x) x == max(x))
label_data <- simulated_data[as.logical(label_index), ]

finalplot <- ggplot2::ggplot(simulated_data, ggplot2::aes(x = AVISITN, y = PCHG, group = USUBJID, color = TRTP, linetype = TRTP, shape = TRTP)) +
  ggplot2::geom_line() +
  ggplot2::geom_point() +
  ggplot2::geom_text(data = label_data, ggplot2::aes(label = USUBJID), nudge_x = 0.35, size = 2.5, show.legend = FALSE) +
  ggplot2::scale_x_continuous("Analysis Visit", breaks = c(0, 4, 8, 12), labels = c("Week 0", "Week 4", "Week 8", "Week 12"), expand = ggplot2::expansion(mult = c(0, 0.18))) +
  ggplot2::scale_y_continuous("Percent Change in FEV1", breaks = seq(-80, 80, 10), limits = c(-80, 80)) +
  ggplot2::scale_color_manual(values = c("Placebo" = "#D55E00", "Drug PILOT" = "#0072B2"), name = "Treatment") +
  ggplot2::scale_linetype_manual(values = c("Placebo" = "dotted", "Drug PILOT" = "solid"), name = "Treatment") +
  ggplot2::scale_shape_manual(values = c("Placebo" = 21, "Drug PILOT" = 22), name = "Treatment") +
  ggplot2::labs(title = "Subject-Level Percent Change in FEV1", subtitle = "Analysis Population") +
  ggplot2::theme_minimal(base_size = 11) +
  ggplot2::theme(panel.grid.minor = ggplot2::element_blank(), panel.grid.major.x = ggplot2::element_blank(), axis.line = ggplot2::element_line(color = "black"), legend.position = "bottom", plot.title = ggplot2::element_text(face = "bold"))
...
write_figure_rtf(png_path, rtf_path, "Subject-Level Percent Change in FEV1", "14.1.7")
