...
# Support running this script from the project root, sdtm folder, or dataset folder.
project_root <- if (file.exists("metadata/sdtm_ct.csv")) {
  normalizePath(".")
} else if (file.exists("../metadata/sdtm_ct.csv")) {
  normalizePath("..")
} else if (file.exists("../../metadata/sdtm_ct.csv")) {
  normalizePath("../..")
} else {
  stop("Cannot find metadata/sdtm_ct.csv. Run this script from the project root, sdtm folder, or dataset folder.")
}

ae_raw <- pharmaverseraw::ae_raw

## ----r------------------------------------------------------------------------
if (!exists("dm", inherits = FALSE)) {
  source(file.path(project_root, "sdtm", "dm", "dm.R"), local = environment())
}
...
## ----r, echo = TRUE-----------------------------------------------------------
study_ct <- read.csv(file.path(project_root, "metadata", "sdtm_ct.csv"))
...
source(file.path(project_root, "sdtm", "write_sdtm_excel.R"))
write_sdtm_excel(
  ae,
  "AE",
  file.path(project_root, "sdtm", "ae", "files", "ae.xlsx")
)
