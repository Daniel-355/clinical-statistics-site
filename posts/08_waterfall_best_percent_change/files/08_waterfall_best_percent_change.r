# =============================================================================
# PILOT clinical figure: Waterfall Plot of Best Percent Change
#
...
stem <- "08_waterfall_best_percent_change"
set.seed(2026082908)
...
original_csv <- paste(c(
  "USUBJID,TRTP,TRTPN,PCHG,BOR",
  "1001-001,Placebo,1,-5,SD",
  "1001-002,Placebo,1,12,PD",
  "1001-003,Placebo,1,-25,PR",
  "1001-004,Placebo,1,-12,SD",
  "1001-005,Placebo,1,-40,PR",
  "1001-006,Placebo,1,8,PD",
  "1001-007,Placebo,1,-55,PR",
  "1001-008,Placebo,1,18,PD",
  "1001-009,Placebo,1,-10,SD",
  "1001-010,Placebo,1,25,PD",
  "1001-011,Drug PILOT,2,-80,CR",
  "1001-012,Drug PILOT,2,-55,PR",
  "1001-013,Drug PILOT,2,-30,PR",
  "1001-014,Drug PILOT,2,-18,SD",
  "1001-015,Drug PILOT,2,-5,SD",
  "1001-016,Drug PILOT,2,10,PD",
  "1001-017,Drug PILOT,2,-70,CR",
  "1001-018,Drug PILOT,2,-50,PR",
  "1001-019,Drug PILOT,2,-25,PR",
  "1001-020,Drug PILOT,2,5,PD"
), collapse = "\n")
...
simulated_data$PCHG <- pmax(-100, pmin(100, simulated_data$PCHG + rnorm(nrow(simulated_data), 0, 4)))
...
# -----------------------------------------------------------------------------
plot_data <- simulated_data[order(-simulated_data$PCHG, simulated_data$USUBJID), ]
plot_data$USUBJID <- factor(plot_data$USUBJID, levels = plot_data$USUBJID)
plot_data$LABEL_Y <- ifelse(plot_data$PCHG > 0, plot_data$PCHG + 4, plot_data$PCHG - 4)

finalplot <- ggplot2::ggplot(plot_data, ggplot2::aes(x = USUBJID, y = PCHG, fill = TRTP)) +
  ggplot2::geom_col() +
  ggplot2::geom_hline(yintercept = c(-30, 20), linetype = "dashed", color = "grey40") +
  ggplot2::geom_text(ggplot2::aes(y = LABEL_Y, label = round(PCHG)), size = 2.4) +
  ggplot2::scale_y_continuous("Best Percent Change", limits = c(-105, 105), breaks = seq(-100, 100, 20), expand = ggplot2::expansion(mult = c(0, 0.03))) +
  ggplot2::scale_fill_manual(values = c("Placebo" = "#9ECAE1", "Drug PILOT" = "#74C476"), name = "Treatment") +
  ggplot2::labs(x = "Subject", title = "Best Percent Change from Baseline", subtitle = "Response-Evaluable Population") +
  ggplot2::theme_classic(base_size = 10) +
  ggplot2::theme(axis.text.x = ggplot2::element_text(angle = -90, vjust = 0.5, hjust = 1, size = 6), legend.position = "bottom", plot.title = ggplot2::element_text(face = "bold"), panel.grid.major.y = ggplot2::element_line(color = "grey80", linetype = "dashed", linewidth = 0.25))
...
write_figure_rtf(png_path, rtf_path, "Waterfall Plot of Best Percent Change", "14.1.8")
